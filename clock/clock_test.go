...
func grid(s tcell.Screen) []string {
	cells, w, h := simscreen.Contents(s)
	out := make([]string, 0, h)
	for y := 0; y < h; y++ {
...
func TestTheClockMoves(t *testing.T) {
	...
	a := grid(func() tcell.Screen { s, _ := at(t, base); return s }())
	b := grid(func() tcell.Screen { s, _ := at(t, base.Add(time.Second)); return s }())
	if strings.Join(a, "\n") == strings.Join(b, "\n") {
