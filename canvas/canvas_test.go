...
func TestFlushPacksTwoPixelsPerCell(t *testing.T) {
	...
	cells, w, _ := simscreen.Contents(screen)
	c := cells[0*w+0]
	if len(c.Runes) == 0 || c.Runes[0] != upperHalf {
		t.Fatalf("cell glyph = %q, want %q", c.Runes, upperHalf)
	}
	fg, bg := c.Style.GetForeground(), c.Style.GetBackground()
	if fg != top {
...
func (h *hiddenScreen) Show()        { h.shown++; h.Screen.Show() }
...
func TestHiddenScreensAreNotDrawn(t *testing.T) {
	...
	h := &hiddenScreen{Screen: sim, active: false}
	a := &countingAnim{}
...
func TestRunHidesTheCursor(t *testing.T) {
	...
	//
	// Written to the queue directly, which v3 documents as the way to inject an
	// event; there is no simulation screen with an InjectKey any more.
